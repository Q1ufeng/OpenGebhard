    ...
    in_gs_sqls.append("""CREATE OR REPLACE FUNCTION borrow_request()\
                            ...
                            BEGIN\
                                ...
                                IF can_borrow THEN\
                                    ...
                                    UPDATE reader_stock\
                                    SET book_num = book_num + subquery.book_num\
                                    FROM (\
                                        SELECT reader_id AS id, book_id, SUM(borrow_num) AS book_num\
                                        FROM temp_joined_table\
                                        GROUP BY reader_id, book_id\
                                    ) AS subquery\
                                    WHERE reader_stock.id = subquery.id\
                                    AND reader_stock.book_id = subquery.book_id;\
